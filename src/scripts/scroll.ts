...
if (navbar) {
  ...
  links.forEach(link => {
    link.addEventListener('click', (e: Event) => {
      ...
      const targetId = (link as HTMLAnchorElement).getAttribute('href'); // Get the target element ID from the link's href attribute
      ...
      if (targetId) {
        ...
        if (targetElement) {
          ...
          const animateScroll = (currentTime: number) => {
            ...
            if (timeElapsed < duration) requestAnimationFrame(animateScroll); // Continue animation if not yet completed
          };
